    ...
    def __init__(self, num_classes=1000):
        ...
        self.features = Sequential([
            ...
            layers.Conv2D(128, kernel_size=5, padding="same",
                          activation="relu"),   # output(None, 27, 27, 128)
            # output(None, 13, 13, 128)
            layers.MaxPool2D(pool_size=3, strides=2),
            layers.Conv2D(192, kernel_size=3, padding="same",
                          activation="relu"),   # output(None, 13, 13, 192)
            layers.Conv2D(192, kernel_size=3, padding="same",
                          activation="relu"),   # output(None, 13, 13, 192)
            layers.Conv2D(128, kernel_size=3, padding="same",
                          activation="relu"),   # output(None, 13, 13, 128)
...
if __name__ == '__main__':
    ...
    model.build((32, 224, 224, 3))  # when using subclass model
    model = AlexNet_v1(num_classes=5)
    model.summary()
    tf.keras.utils.plot_model(model, to_file='AlexNet.png', show_shapes=True,
